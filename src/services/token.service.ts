...
import { AccessToken, AuthTokensResponse } from '../types/response';
 ...
 * @returns A Promise that resolves to the extracted payload or an unknown value.
 */

const getPayload = async (token: string, type: TokenType): Promise<AccessToken> => {
  const payload = jwt.verify(token, config.jwt.secret);
